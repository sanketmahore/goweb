...
func CreateBooking(booking *domain.Booking) error{
	err := db.Create(booking).Error
	return err
}

func ReturnAllBookings() []*domain.Booking {
	bookings := make([]*domain.Booking, 0)
	db.Find(&bookings)
	return bookings
}

func ReturnSingleBooking(id string) (*domain.Booking, error){
...
func UpdateBooking(booking *domain.Booking) error{
